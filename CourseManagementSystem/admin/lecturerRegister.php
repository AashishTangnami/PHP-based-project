...
&& in_array($extension, $allowedExts)) #condition is checked
  ...
  if ($_FILES["image"]["error"] > 0) #condition is checked
    ...
    if (file_exists("lecturer_image/" . $_FILES["image"]["name"])) #condition is checked
      {
...
$criteria = [
						...
						$sql =  $pdo->prepare("INSERT INTO lecturer_table(lecturer_email, lecturer_password, fullName, phoneNumber, lecturer_Address, lecturer_pic, lecturer_picName) VALUES ('$student_email', '$passwordHashed', '$fullName', '$phoneNumber','$student_Address','$img', '$image')
...
<pre>
	<form action="" method="POST" enctype="multipart/form-data">
			<fieldset>
					<legend><h2>Lecturer Register</h2></legend>
				<label>Email : </label>
				...
				<input type="text" name="fullName" placeholder="you email" required autofocus></input><br><br>
				<label>Phone Number : </label>
				<input type="number" name="phoneNumber" placeholder="you email" required autofocus></input><br><br>
				<label>Address: </label>
				<input type="text" name="student_Address" placeholder="you email" required autofocus></input><br><br>
				<label>Image</label>
